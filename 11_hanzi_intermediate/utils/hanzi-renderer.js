...
class HanziRenderer {
  ...
  async _fetchCharData(char) {
    const raw = this.dataSource[char];
    if (!raw) {
      throw new Error(`No stroke data for character: ${char}`);
    }
    // Transform compact format (s/m) to renderer format (strokes/medians)
    // m is flat arrays [x1,y1,x2,y2,...], convert to [[x1,y1],[x2,y2],...]
    const medians = raw.m ? raw.m.map(function(flat) {
      var pts = [];
