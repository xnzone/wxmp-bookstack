...
If you've done any programming, the idea of levels of abstraction is probably familiar to you. You'll always work at some level of abstraction, interface with a lower level layer through some API, and probably provide some higher-level API or user interface to your users. The seven-layer [OSI model of computer networking](https://en.wikipedia.org/wiki/OSI_model) is a good example of this.
...
Another alternative is to assume that nodes can fail by misbehaving in any arbitrary way. This is known as [Byzantine fault tolerance](https://en.wikipedia.org/wiki/Byzantine_fault_tolerance). Byzantine faults are rarely handled in real world commercial systems, because algorithms resilient to arbitrary faults are more expensive to run and more complex to implement. I will not discuss them here.
...
Of course, in order to have a discussion, we also need to introduce a problem to solve. The problem I'm going to discuss is the [consensus problem](https://en.wikipedia.org/wiki/Consensus_%28computer_science%29).
...
First, that _many system designs used in early distributed relational database systems did not take into account partition tolerance_ (e.g. they were CA designs). Partition tolerance is an important property for modern systems, since network partitions become much more likely if the system is geographically distributed (as many large systems are).

Second, that _there is a tension between strong consistency and high availability during network partitions_. The CAP theorem is an illustration of the tradeoffs that occur between strong guarantees and distributed computation.
...
![](https://s2.loli.net/2024/11/18/e6f95sSXv2uAIOM.jpg)
...
> [ACID](https://en.wikipedia.org/wiki/ACID) consistency !=  
> [CAP](https://en.wikipedia.org/wiki/CAP_theorem) consistency !=  
> [Oatmeal](https://en.wikipedia.org/wiki/Oatmeal) consistency
